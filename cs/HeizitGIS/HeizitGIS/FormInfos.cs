...
namespace HeizitGIS
{
    public partial class FormInfos : Form
    {
        ...
        private void UpdateReceiveMsg()
        {
            ...
            string sql = String.Format("SELECT FromName, SendTime, IsRead, [Object-ID] FROM ViewMsgs WHERE ToID = {0}", _fromID);
            DataTable dt = SQLHelper.GetDataTable(sql);
            int len = dt.Rows.Count;
            for (int i = 0; i < len; i++)
            {
        ...
        private void UpdateSendMsg()
        {
            ...
            string sql = String.Format("SELECT ToName, SendTime, IsRead, [Object-ID] FROM ViewMsgs WHERE FromID = {0}", _fromID);
            DataTable dt = SQLHelper.GetDataTable(sql);
            int len = dt.Rows.Count;
            for (int i = 0; i < len; i++)
            {
        ...
        private void btn_SendRead_Click(object sender, EventArgs e)
        {
            ...
            string sql = String.Format("UPDATE Msgs SET IsRead = 1 WHERE [Object-ID] = {0}", obgID);
            if (SQLHelper.CommandSQL(sql) == 1)
            {
        ...
        private void listView_receivesend_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = String.Format("SELECT Msg FROM Msgs WHERE [Object-ID] = {0}", obgID);
                DataTable dt = SQLHelper.GetDataTable(sql);
